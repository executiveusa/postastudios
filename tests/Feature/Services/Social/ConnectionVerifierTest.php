...
test('verifies account without refresh when token is not expired', function () {
    Http::fake([
        config('postastudio.platforms.linkedin.api').'/*' => Http::response(['sub' => '123'], 200),
    ]);
    ...
    Http::assertSent(fn ($request) => str_contains($request->url(), config('postastudio.platforms.linkedin.api').'/rest/userinfo'));
});
...
test('refreshes linkedin token before verifying when expired', function () {
    ...
    Http::assertSent(fn ($request) => str_contains($request->url(), config('postastudio.platforms.linkedin.oauth_api').'/oauth/v2/accessToken'));
});
...
test('refreshes x token before verifying when expired', function () {
    ...
    Http::assertSent(fn ($request) => str_contains($request->url(), config('postastudio.platforms.x.api').'/oauth2/token'));
});

test('refreshes bluesky token before verifying when expired', function () {
    Http::fake([
        config('postastudio.platforms.bluesky.default_service').'/xrpc/com.atproto.server.refreshSession' => Http::response([
            'accessJwt' => 'new_access_token',
            'refreshJwt' => 'new_refresh_token',
        ], 200),
        config('postastudio.platforms.bluesky.default_service').'/xrpc/app.bsky.actor.getProfile*' => Http::response(['did' => 'did:plc:123'], 200),
    ]);
...
test('refreshes youtube token before verifying when expired', function () {
    Http::fake([
        config('postastudio.platforms.youtube.oauth_api').'/token' => Http::response([
            'access_token' => 'new_token',
            'expires_in' => 3600,
        ], 200),
        config('postastudio.platforms.youtube.data_api').'/*' => Http::response(['items' => []], 200),
    ]);
    ...
    Http::assertSent(fn ($request) => str_contains($request->url(), config('postastudio.platforms.youtube.oauth_api').'/token'));
});

test('refreshes tiktok token before verifying when expired', function () {
    Http::fake([
        config('postastudio.platforms.tiktok.api').'/oauth/token/' => Http::response([
            'access_token' => 'new_token',
            'refresh_token' => 'new_refresh_token',
            'expires_in' => 86400,
        ], 200),
        config('postastudio.platforms.tiktok.api').'/user/info/*' => Http::response(['data' => ['user' => []]], 200),
    ]);
    ...
    Http::assertSent(fn ($request) => str_contains($request->url(), config('postastudio.platforms.tiktok.api').'/oauth/token'));
});

test('refreshes pinterest token before verifying when expired', function () {
    Http::fake([
        config('postastudio.platforms.pinterest.api').'/oauth/token' => Http::response([
            'access_token' => 'new_token',
            'refresh_token' => 'new_refresh_token',
            'expires_in' => 86400,
        ], 200),
        config('postastudio.platforms.pinterest.api').'/user_account' => Http::response(['username' => 'test'], 200),
    ]);
    ...
    Http::assertSent(fn ($request) => str_contains($request->url(), config('postastudio.platforms.pinterest.api').'/oauth/token'));
});
...
test('refreshes threads token before verifying when expired', function () {
    Http::fake([
        ...
        config('postastudio.platforms.threads.graph_api').'/me*' => Http::response(['id' => '123', 'username' => 'test'], 200),
    ]);
...
test('throws exception when linkedin refresh fails', function () {
    Http::fake([
        config('postastudio.platforms.linkedin.oauth_api').'/oauth/v2/accessToken' => Http::response(['error' => 'invalid_grant'], 400),
    ]);
...
test('does not refresh facebook token as it uses long-lived tokens', function () {
    Http::fake([
        config('postastudio.platforms.facebook.graph_api').'/*' => Http::response(['id' => '123', 'name' => 'Test'], 200),
    ]);
    ...
    Http::assertSentCount(1);
    Http::assertSent(fn ($request) => str_contains($request->url(), config('postastudio.platforms.facebook.graph_api')));
});
...
test('refreshes instagram token when expired', function () {
    Http::fake([
        ...
        config('postastudio.platforms.instagram.graph_api').'/me*' => Http::response(['id' => '123', 'username' => 'test'], 200),
    ]);
...
test('refreshes lazily on 401 then retries verify', function () {
    Http::fake([
        ...
        config('postastudio.platforms.linkedin.api').'/*' => Http::sequence()
...
test('throws when verify returns 401 AND refresh also fails', function () {
    Http::fake([
        config('postastudio.platforms.linkedin.api').'/*' => Http::response(['error' => 'unauthorized'], 401),
        config('postastudio.platforms.linkedin.oauth_api').'/oauth/v2/accessToken' => Http::response(['error' => 'invalid_grant'], 400),
    ]);
...
test('connection failure during refresh raises PlatformUnavailableException', function () {
    Http::fake([
        config('postastudio.platforms.youtube.oauth_api').'/token' => fn () => throw new ConnectionException('cURL error 7: connection refused'),
    ]);
...
test('does not disconnect when the verify after a refresh 401s once but a fresh token is available', function () {
    Http::fake([
        ...
        config('postastudio.platforms.x.api').'/users/me' => Http::sequence()
            ->push(['error' => 'unauthorized'], 401)
...
test('429 during refresh raises PlatformUnavailableException (rate limit is transient)', function () {
    Http::fake([
        config('postastudio.platforms.x.api').'/oauth2/token' => Http::response(['error' => 'rate_limit_exceeded'], 429),
    ]);
...
test('verifies discord by checking the bot is still in the guild', function () {
    ...
    Http::fake([
        config('postastudio.platforms.discord.api').'/guilds/*' => Http::response(['id' => '999000111'], 200),
    ]);
...
test('throws TokenExpiredException when the discord bot was removed from the guild', function () {
    ...
    Http::fake([
        config('postastudio.platforms.discord.api').'/guilds/*' => Http::response(['message' => 'Unknown Guild'], 404),
    ]);
...
test('throws TokenExpiredException when the discord bot is no longer in the guild (403)', function () {
    ...
    Http::fake([
        config('postastudio.platforms.discord.api').'/guilds/*' => Http::response(['message' => 'Missing Access', 'code' => 50001], 403),
    ]);
...
test('throws PlatformUnavailableException, not TokenExpiredException, when the shared discord bot token is rejected', function () {
    ...
    Http::fake([
        config('postastudio.platforms.discord.api').'/guilds/*' => Http::response(['message' => '401: Unauthorized'], 401),
    ]);
...
test('instagram verify treats a Meta rate-limit (OAuthException code 4) as transient, not a disconnect', function () {
    Http::fake([
        config('postastudio.platforms.instagram.graph_api').'/me*' => Http::response([
            'error' => ['message' => 'Application request limit reached', 'type' => 'OAuthException', 'code' => 4],
        ], 400),
    ]);
...
test('threads verify treats a dead token reported under a non-190 code as genuinely expired', function () {
    ...
    Http::fake([
        config('postastudio.platforms.threads.graph_api').'/me*' => Http::response([
...
test('threads verify treats a 5xx as platform unavailable, not a disconnect', function () {
    Http::fake([
        config('postastudio.platforms.threads.graph_api').'/me*' => Http::response('upstream timeout', 503),
    ]);
...
test('facebook verify treats a dead token reported under a non-190 code as genuinely expired', function () {
    Http::fake([
        config('postastudio.platforms.facebook.graph_api').'/me*' => Http::response([
            'error' => ['message' => 'The requested resource does not exist', 'type' => 'OAuthException', 'code' => 100],
        ], 400),
    ]);
...
test('facebook verify treats a 5xx as platform unavailable, not a disconnect', function () {
    Http::fake([
        config('postastudio.platforms.facebook.graph_api').'/me*' => Http::response('upstream timeout', 503),
    ]);
...
test('facebook verify treats a non-JSON failure body as platform unavailable, not a confirmed dead token', function () {
    Http::fake([
        config('postastudio.platforms.facebook.graph_api').'/me*' => Http::response('<html>blocked</html>', 400),
    ]);
...
test('instagram verify treats a 5xx as platform unavailable, not a disconnect', function () {
    Http::fake([
        config('postastudio.platforms.instagram.graph_api').'/me*' => Http::response('upstream timeout', 503),
    ]);
...
test('threads verify treats a non-JSON failure body as platform unavailable, not a confirmed dead token', function () {
    ...
    Http::fake([
        config('postastudio.platforms.threads.graph_api').'/me*' => Http::response('<html>blocked</html>', 400),
    ]);
...
test('threads verify treats a failed response with a valid but unrecognized JSON shape as a confirmed rejection', function () {
    ...
    Http::fake([
        config('postastudio.platforms.threads.graph_api').'/me*' => Http::response(['data' => ['id' => '123']], 400),
    ]);
...
test('threads refresh treats a non-JSON failure body as platform unavailable, not a confirmed dead token', function () {
    Http::fake([
        config('postastudio.platforms.threads.auth_api').'/refresh_access_token*' => Http::response('<html>blocked</html>', 400),
    ]);
...
test('instagram refresh treats a non-JSON failure body as platform unavailable, not a confirmed dead token', function () {
    Http::fake([
        config('postastudio.platforms.instagram.auth_api').'/refresh_access_token*' => Http::response('<html>blocked</html>', 400),
    ]);
...
test('throws PlatformUnavailableException when the discord guild lookup fails transiently', function () {
    ...
    Http::fake([
        config('postastudio.platforms.discord.api').'/guilds/*' => Http::response(['message' => 'Internal Server Error'], 500),
    ]);
...
test('linkedin verify treats a 5xx as platform unavailable, not a disconnect', function () {
    Http::fake([
        config('postastudio.platforms.linkedin.api').'/rest/userinfo' => Http::response(['message' => 'Internal Server Error'], 500),
    ]);
...
test('linkedin verify throws TokenExpiredException on a bare 401', function () {
    Http::fake([
        config('postastudio.platforms.linkedin.oauth_api').'/oauth/v2/accessToken' => Http::response(['error' => 'invalid_grant'], 400),
        config('postastudio.platforms.linkedin.api').'/rest/userinfo' => Http::response(['message' => 'Unauthorized'], 401),
    ]);
...
test('linkedin page verify treats a 5xx as platform unavailable, not a disconnect', function () {
    Http::fake([
        config('postastudio.platforms.linkedin-page.api').'/rest/organizationAcls*' => Http::response(['message' => 'Internal Server Error'], 500),
    ]);
...
test('linkedin page verify throws TokenExpiredException on a bare 401', function () {
    Http::fake([
        config('postastudio.platforms.linkedin.oauth_api').'/oauth/v2/accessToken' => Http::response(['error' => 'invalid_grant'], 400),
        config('postastudio.platforms.linkedin-page.api').'/rest/organizationAcls*' => Http::response(['message' => 'Unauthorized'], 401),
    ]);
...
test('x verify treats a 5xx as platform unavailable, not a disconnect', function () {
    Http::fake([
        config('postastudio.platforms.x.api').'/users/me' => Http::response(['title' => 'Internal Server Error'], 503),
    ]);
...
test('youtube verify treats a 5xx as platform unavailable, not a disconnect', function () {
    Http::fake([
        config('postastudio.platforms.youtube.data_api').'/channels*' => Http::response(['error' => ['message' => 'Backend Error']], 500),
    ]);
...
test('youtube verify throws TokenExpiredException on a bare 401', function () {
    Http::fake([
        config('postastudio.platforms.youtube.oauth_api').'/token' => Http::response(['error' => 'invalid_grant'], 400),
        config('postastudio.platforms.youtube.data_api').'/channels*' => Http::response(['error' => ['message' => 'Unauthorized']], 401),
    ]);
...
test('pinterest verify treats a 5xx as platform unavailable, not a disconnect', function () {
    Http::fake([
        config('postastudio.platforms.pinterest.api').'/user_account' => Http::response(['message' => 'Internal Server Error'], 500),
    ]);
...
test('pinterest verify throws TokenExpiredException on a bare 401', function () {
    Http::fake([
        config('postastudio.platforms.pinterest.api').'/oauth/token' => Http::response(['error' => 'invalid_grant'], 400),
        config('postastudio.platforms.pinterest.api').'/user_account' => Http::response(['message' => 'Unauthorized'], 401),
    ]);
...
test('tiktok verify treats a 5xx as platform unavailable, not a disconnect', function () {
    Http::fake([
        config('postastudio.platforms.tiktok.api').'/user/info/*' => Http::response(['error' => ['code' => 'internal_error']], 500),
    ]);
...
test('tiktok verify throws TokenExpiredException on a bare 401 from user/info', function () {
    ...
    Http::fake([
        ...
        config('postastudio.platforms.tiktok.api').'/user/info/*' => Http::response(['error' => ['code' => 'access_token_invalid']], 401),
    ]);
...
test('tiktok verify throws TokenExpiredException on a bare 401 with no recognized error code', function () {
    ...
    Http::fake([
        ...
        config('postastudio.platforms.tiktok.api').'/user/info/*' => Http::response(['error' => ['code' => 'access_token_revoked']], 401),
    ]);
...
test('bluesky verify treats a 5xx as platform unavailable, not a disconnect', function () {
    Http::fake([
        config('postastudio.platforms.bluesky.default_service').'/xrpc/*' => Http::response(['message' => 'Internal Server Error'], 500),
    ]);
...
test('bluesky verify throws TokenExpiredException when getProfile reports ExpiredToken', function () {
    Http::fake([
        ...
        config('postastudio.platforms.bluesky.default_service').'/xrpc/com.atproto.server.refreshSession' => Http::response(['error' => 'ExpiredToken'], 400),
        config('postastudio.platforms.bluesky.default_service').'/xrpc/com.atproto.server.createSession' => Http::response(['error' => 'InvalidRequest'], 400),
        config('postastudio.platforms.bluesky.default_service').'/xrpc/app.bsky.actor.getProfile*' => Http::response(['error' => 'ExpiredToken'], 400),
    ]);
